...
def forward_check_and_propagate(state, color, usa_map, available_colors, assignment):
    updates = {}
    for neighbor in usa_map[state]:
        if neighbor not in assignment and color in available_colors[neighbor]:
            ...
            if len(available_colors[neighbor]) == 1:
                ...
                if not forward_check_and_propagate(neighbor, singleton_color, usa_map, available_colors, assignment):
                    return False
...
def dfs_with_heuristics_forward_check_and_propagation(usa_map, colors, assignment, available_colors, backtrack_count):
    ...
    for color in order_domain_values(state, usa_map, available_colors):
        if color in available_colors[state]:
            ...
            if forward_check_and_propagate(state, color, usa_map, available_colors, assignment):
                result, backtrack_count = dfs_with_heuristics_forward_check_and_propagation(usa_map, colors, assignment, available_colors, backtrack_count)
...
def map_coloring_with_heuristics_forward_check_and_propagation(usa_map, colors):
    start_time = time.time()
    assignment = {}
    available_colors = {state: set(colors) for state in usa_map}
    solution, backtrack_count = dfs_with_heuristics_forward_check_and_propagation(usa_map, colors, assignment, available_colors, 0)
    end_time = time.time()
...
coloring, backtracks, time_taken = map_coloring_with_heuristics_forward_check_and_propagation(usa_map, colors)
